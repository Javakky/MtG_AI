...
def get_sample_deck() -> List[Card]:
    return flatten([
        ...
        CARD_POOL.get_card("わら人形の兵士", 2),
        CARD_POOL.get_card("フェメレフの斥候", 2),
        CARD_POOL.get_card("アラボーンの強兵", 2),
        CARD_POOL.get_card("ゴリラの戦士", 2),
        CARD_POOL.get_card("ナナカマドのツリーフォーク", 2),
        CARD_POOL.get_card("トカゲ人間の戦士", 2),
        CARD_POOL.get_card("骸骨クロコダイル", 2),
        CARD_POOL.get_card("板金鎧のワーム"),
        CARD_POOL.get_card("針刺ワーム"),
        CARD_POOL.get_card("黒曜石のゴーレム"),
        CARD_POOL.get_card("ヴィザードリックス"),
